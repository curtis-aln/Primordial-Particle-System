...
{
	if (rend_map)
		render_heat_map(snapshot, camera);
	if (rend_particles)
		render_particles(snapshot, camera);
}

void PPS_Renderer::render_heat_map(const SimSnapshot& snapshot, const Camera& camera)
{
	heatmap.clear();
...
void PPS_Renderer::render_particles(const SimSnapshot& snapshot, const Camera& camera)
{
	...
	const float alpha = zoom_to_alpha(camera.m_current_zoom_, zoom_min, zoom_max, alpha_min, alpha_max);
