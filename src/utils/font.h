...
public:
	...
	Font(sf::RenderWindow* window, const unsigned font_size, const std::string& font_location) : m_window_(window), m_text_(m_font_)
    {
    ...
    void draw(const sf::Vector2f& position, const std::string& string_text, const bool centered = false)
    {
        ...
        if (centered)
        {
            text_position.x -= text_bounds.size.x / 2.0f;
            text_position.y -= text_bounds.size.y / 2.0f;
        }
